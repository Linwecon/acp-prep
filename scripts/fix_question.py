...
REPLACEMENTS = {
    ...
    "3-0215": {"type": 1},
    "7-0160": {"type": 1},
    # 5-0120：题干"内部知识查询"有歧义（模型内部知识 vs 企业私有知识）+ 缺解析，改写并补解析
    "5-0120": {
        "stem": "以下哪项任务可以直接交给大模型完成，无需检索外部知识库或额外训练？",
        "options": [
            {"option_label": "A", "option_text": "文档审查（把文档交给模型并给出审阅意见）"},
            {"option_label": "B", "option_text": "企业私有知识库问答"},
            {"option_label": "C", "option_text": "增加模型的训练数据"},
            {"option_label": "D", "option_text": "减少模型的推理时间"},
        ],
        "answer": "A",
        "analysis": "文档审查是大模型的直接适用场景：把文档连同审阅指令一起输入，模型即可生成审阅意见（大纲中的“用大模型做文档审阅”）。企业私有知识库问答则不同——私有数据不在模型参数里，需通过 RAG 先检索再生成，不能“直接输入”得到答案；增加训练数据属于训练环节操作，减少推理时间属于部署/性能优化，二者都不是“输入内容生成答案”的任务。",
    },
